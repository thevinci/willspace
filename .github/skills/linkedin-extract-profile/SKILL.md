...
// --- Basic info ---
const name      = await summaryEl.locator('> div h2').first().innerText().catch(() => null);
const headline  = await summaryEl.locator('> p:nth-child(2)').first().innerText().catch(() => null);
const thirdLine = await summaryEl.locator('> p:nth-child(3)').first().innerText().catch(() => null);
const [company, college] = (thirdLine ?? '').split(' · ').map(s => s.trim());
const location  = await contactParentEl.locator('> p').first().innerText().catch(() => null);

// --- Profile image: pick the largest (last) entry from srcset ---
const srcset       = await profileImageEl.getAttribute('srcset').catch(() => null);
const profileImage = srcset?.split(',').map(s => s.trim().split(' ')[0]).pop() ?? null;

// --- About section ---
const about = await aboutEl
  .locator('span[data-testid="expandable-text-box"]').first()
...
const getDialogField = (label) =>
  dialogEl.locator(`p:has-text("${label}") + p`).first().innerText().catch(() => null);
...
// Dismiss dialog
await page.locator('dialog[open] button[aria-label="Dismiss"]').first().click().catch(() => {});
